...
dt <- read.csv2("data/children2.csv", header = TRUE)
str(dt)
summary(dt)
M <- as.matrix(dt)
View(dt)
...
model.M <- lm(IQ ~ Memory + Age + Reading, data = dt)
cor(dt$IQ, fitted.values(model.M)) # by definition, as correlation between variable and its best linear approximation
